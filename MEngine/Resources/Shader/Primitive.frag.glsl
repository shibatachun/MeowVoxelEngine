...
layout(binding = 1) uniform texture2D g_BaseColorTexture;
layout(binding = 2) uniform sampler g_BaseColorSampler;
...
void main()
{
    ...
    float disablePrimitiveEdges = clamp(gbuffer.materialParameters.w, 0.0, 1.0);
    vec3 textureAlbedo = texture(sampler2D(g_BaseColorTexture, g_BaseColorSampler), v_FaceUV).rgb;
    vec3 baseAlbedo = textureAlbedo * clamp(v_Albedo, vec3(0.0), vec3(1.0));
    vec3 edgedAlbedo = mix(baseAlbedo, vec3(0.015), edgeMask * 0.82 * (1.0 - disablePrimitiveEdges));
